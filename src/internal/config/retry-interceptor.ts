...
export function addRetryInterceptor(
  ...
  axiosInstance.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      ...
      if (serverDelay !== null) {
        // Respect server-suggested delay without capping — the server
        // knows the cooldown duration (e.g. 60s) and capping would
        // cause premature retries that get rejected again.
        retryDelay = serverDelay;

        if (retryConfig.enableLogging) {
          retryConfig.logger.log(
            `[Capsara SDK] Retry attempt ${requestConfig.__retryCount}/${retryConfig.maxRetries} ` +
            `for ${status} error - waiting ${retryDelay}ms (server suggested)`
          );
        }
      } else {
        ...
        if (retryConfig.enableLogging) {
          retryConfig.logger.log(
            `[Capsara SDK] Retry attempt ${requestConfig.__retryCount}/${retryConfig.maxRetries} ` +
            `for ${status} error - waiting ${retryDelay}ms (exponential backoff)`
          );
        }
      }
