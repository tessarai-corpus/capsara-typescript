...
describe('Golden: Retry', () => {
  ...
  it('should respect server Retry-After without capping at maxDelay', async () => {
    const { instance, getErrorHandler } = createMockAxiosInstance();
    const mockRequest = vi.fn().mockResolvedValue({ data: 'ok', status: 200 });
    (instance as { request: typeof mockRequest }).request = mockRequest;

    // maxDelay = 2000, but Retry-After header says 60 seconds
    // Server-suggested delays should NOT be capped — server knows the cooldown duration
    ...
    // Should NOT have retried at 2500ms (would have if capped at maxDelay)
    await vi.advanceTimersByTimeAsync(2500);
    expect(mockRequest).not.toHaveBeenCalled();

    // Should retry after full 60s (60000ms)
