...
describe('retry-interceptor', () => {
  ...
  describe('retry behavior for 429 errors', () => {
    it('should retry on 429 with Retry-After header', async () => {
      ...
      const error = createMockAxiosError(429, {}, { 'retry-after': '1' });
      const errorHandler = getResponseErrorHandler();
    ...
    it('should reject immediately on 429 without Retry-After', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
  ...
  describe('Retry-After header handling', () => {
    ...
    it('should NOT cap Retry-After to maxDelay for 429 (server delay respected)', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
      const mockRequest = vi.fn().mockResolvedValue({ data: 'success', status: 200 });
      (instance as { request: typeof mockRequest }).request = mockRequest;
      ...
      // Server suggests 60 seconds and maxDelay is 5 seconds — server wins for 429
      const error = createMockAxiosError(429, {}, { 'retry-after': '60' });
      const errorHandler = getResponseErrorHandler();

      const retryPromise = errorHandler!(error);

      // Should NOT have retried at 5 seconds (the maxDelay cap)
      await vi.advanceTimersByTimeAsync(5000);
      expect(mockRequest).not.toHaveBeenCalled();

      // Should retry after the full 60 seconds
  ...
  describe('edge cases', () => {
    it('should reject 429 with invalid Retry-After header (no valid delay)', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
      const mockRequest = vi.fn();
      (instance as { request: typeof mockRequest }).request = mockRequest;
      ...
      // Invalid date string — no valid Retry-After means 429 is rejected immediately
    ...
    it('should reject 429 with non-number retryAfter in response body', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
      const mockRequest = vi.fn();
      (instance as { request: typeof mockRequest }).request = mockRequest;
      ...
      const responseData = {
        error: {
          retryAfter: 'five', // String instead of number — not a valid delay
          message: 'Rate limited',
        },
      };
      const error = createMockAxiosError(429, responseData);
      ...
      // No valid Retry-After means 429 is rejected immediately
      await expect(errorHandler!(error)).rejects.toThrow();
      expect(mockRequest).not.toHaveBeenCalled();
    });

    it('should reject 429 with null response data (no Retry-After)', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
      const mockRequest = vi.fn();
      (instance as { request: typeof mockRequest }).request = mockRequest;
      ...
      // No Retry-After header or body — 429 rejected immediately
      await expect(errorHandler!(error)).rejects.toThrow();
      expect(mockRequest).not.toHaveBeenCalled();
    });

    it('should reject 429 with response data missing error field (no Retry-After)', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
      const mockRequest = vi.fn();
      (instance as { request: typeof mockRequest }).request = mockRequest;
      ...
      // No Retry-After header or body retryAfter — 429 rejected immediately
      await expect(errorHandler!(error)).rejects.toThrow();
      expect(mockRequest).not.toHaveBeenCalled();
    });

    it('should reject 429 with error.error being null (no Retry-After)', async () => {
      const { instance, getResponseErrorHandler } = createMockAxiosInstance();
      const mockRequest = vi.fn();
      (instance as { request: typeof mockRequest }).request = mockRequest;
      ...
      // No valid Retry-After — 429 rejected immediately
      await expect(errorHandler!(error)).rejects.toThrow();
      expect(mockRequest).not.toHaveBeenCalled();
    });
  });
